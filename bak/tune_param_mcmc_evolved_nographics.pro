...
!p.multi=0
	...
	global_priors_names=['model_Evolved_Global_a1etaa3_l1mixed', 'bool', 'Uniform', 'Fix_Auto', $
					     'Fix', 'Fix', 'Uniform', $
					     'Gaussian', 'Gaussian', 'Gaussian', 'Jeffreys', 'Fix_Auto']
	...
	global_priors_params[where(global_params_names eq 'Width'), 0]=1
	global_priors_params[where(global_params_names eq 'Height'), 0:1]=[1, 10000.]
	...
	resol=freq[1] - freq[0]
	gamma0=mean(params[total(parameters_length[0:6]): total(parameters_length[0:7])-1]);/2.
	scoef=gamma0/resol
	...
	nu=dblarr(lmax+1, Nmax)
	for el=0, lmax do begin
		pos0=total(parameters_length[0:1+el])
		pos1=total(parameters_length[0:1+el+1]) -1
		if parameters_length[1+el+1] ne 0 then nu[el,0:parameters_length[1+el+1]-1]=params[pos0:pos1] ; Fill this only if there is modes
	endfor
	...
	params[total(parameters_length[0:6]): total(parameters_length[0:7])-1]=widths[0,*]	
...
key='n' & status=0
	...
	; Inclination:
	global_priors_params[where(global_params_names eq 'Inclination'), 0:2]=[inc, 0., incmax]
	...
	;els=dblarr(max([n_elements(posl0), n_elements(posl1), n_elements(posl2), n_elements(posl3)]))
	els=dblarr(n_elements(posl0))
	if posl0[0] ne -1 then begin
	...
	if lmax ge 1 and posl1[0] ne -1 then begin
	...
	if lmax ge 2 and posl2[0] ne -1 then begin	
	...
	if lmax ge 3 and posl3[0] ne -1 then begin
...
; ******** properties of the frequencies ********
	...
	posl0=where(els eq 0)
	posl1=where(els eq 1)
	posl2=where(els eq 2)
	posl3=where(els eq 3)

	if posl0[0] ne -1 then fl0=freq_l[posl0]
	if lmax ge 1 and posl1[0] ne -1 then fl1=freq_l[posl1]
	if lmax ge 2 and posl2[0] ne -1 then fl2=freq_l[posl2]
	if lmax ge 3 and posl3[0] ne -1 then fl3=freq_l[posl3]
...
; **************** Parameters for the eigen solutions ************
	...
	for i=0, n_elements(global_params_names)-1 do begin
		...
		j=0
		if pos_OK[0] ne -1 then if global_priors_params[i,pos_OK[j]] ne -9999 then chain=chain + string(global_priors_params[i,pos_OK[j]], format='(f18.6)')
