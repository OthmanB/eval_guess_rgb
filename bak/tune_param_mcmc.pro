...
@build_init_param_a1a2a3.pro
@global_fit_a1a2a3.pro
;@star_mode_ID.pro
pro tune_param_mcmc, kic_n, data_type
!p.multi=0
	...
	global_priors_names=['model_MS_Global_a1etaa3_HarveyLike', 'bool', 'Uniform', 'Fix_Auto', $
					     'Fix', 'Fix', 'Uniform_cos', $
					     'Gaussian', 'Gaussian', 'Gaussian']
	global_priors_params=dblarr(n_elements(global_priors_names), 5)
	global_priors_params[*,*]=-9999
	
	global_priors_params[where(global_params_names eq 'freq_smoothness'), 0]=1 ; Activate/Deactivate the smoothness condition on frequencies
	...
	if n_elements(file_out_res) eq 1 then begin
		struc=build_init_param_a1a2a3(file_out_res)
	endif else begin
		...
		read, a, format='(i)', prompt='Enter the index of the file: '
		struc=build_init_param_a1a2a3(file_out_res[a])
	endelse	
...
;				3.7507970, 2.8629352, 1.8167902, 0.92533429, 0.42467669, 0.17490098, 0.079882521, 0.038872344]
	...
	;if Nmax/2. - floor(Nmax/2.) ne 0 then begin
	;	print, 'Nmax must be a odd number. Please change that value accordingly'
